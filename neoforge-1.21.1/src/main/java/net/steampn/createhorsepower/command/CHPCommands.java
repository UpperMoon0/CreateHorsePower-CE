...
public class CHPCommands {
    ...
    private static int inspectTargetCrank(CommandContext<CommandSourceStack> context) {
        ...
        if (!(player.level().getBlockEntity(pos) instanceof HorseCrankTileEntity crank)) {
    ...
    private static int inspectWorkerType(CommandContext<CommandSourceStack> context) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        CommandSourceStack source = context.getSource();
        ...
        if (statsOpt.isEmpty()) {
            source.sendFailure(Component.literal("No WorkerStats profile found for " + BuiltInRegistries.ENTITY_TYPE.getKey(type)));
            return 0;
        }

        WorkerStats stats = statsOpt.get();
        source.sendSuccess(() -> Component.literal("=== Worker Profile: " + BuiltInRegistries.ENTITY_TYPE.getKey(type) + " ===").withStyle(ChatFormatting.GOLD), false);
    ...
    private static int inspectPathBlock(CommandContext<CommandSourceStack> context) throws com.mojang.brigadier.exceptions.CommandSyntaxException {
        CommandSourceStack source = context.getSource();
        ...
        if (statsOpt.isEmpty()) {
            source.sendFailure(Component.literal("Block " + BuiltInRegistries.BLOCK.getKey(block) + " is not a valid Horse Crank path material."));
            return 0;
        }

        PathStats stats = statsOpt.get();
        source.sendSuccess(() -> Component.literal("=== Path Material: " + BuiltInRegistries.BLOCK.getKey(block) + " ===").withStyle(ChatFormatting.GOLD), false);
